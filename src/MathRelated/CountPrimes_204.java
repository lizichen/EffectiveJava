...
public class CountPrimes_204 {
    ...
    public static int countPrimes2(int n) {
        boolean[] notPrime = new boolean[n];    // first assume all numbers are prime; False for all notPrime
        int count = 0;
        for (int i = 2; i < n; i++) {
            if (notPrime[i] == false) {         // i is not a notPrime, meaning, i is a prime
                                                // starting from i = 2, 3, will make sure the 4, 6 has been marked as true
                count++;
                for (int j = 2; i*j < n; j++) {
                    notPrime[i*j] = true;
                }
